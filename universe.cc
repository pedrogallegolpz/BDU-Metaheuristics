...
/*
    Genera un vector ortonormal a 'v' y lo guarda en 'orthogonal'.
*/
void generate_orthonormal(std::vector<double> &v, std::vector<double> &orthogonal, int seed){
    ...
    int coord = dis(gen)%v.size();


    int aux=0;
    while(v[coord]==0 && aux<v.size()){
        aux++;
        coord = (coord+1)%v.size();
    }

    // Salvamos que un vector de movimiento
    if(aux==v.size() && v[coord]==0){
        v[coord]=1;
    }
    ...
    for(int i=0; i< v.size(); i++){
        if(i!=coord){
            orthogonal[i] = 1;  // Asignamos un 1 a todas las coordenadas menos una, con la que forzaremos la ortogonalidad
            sum += v[i];    
        }
    }

    // Forzamos la ortogonalidad
    orthogonal[coord] = -(sum/v[coord]);

    // Forzamos que sea unitario
    double norma = l2_norm(orthogonal);
    for(int i=0; i<orthogonal.size();i++){
        orthogonal[i] = orthogonal[i]/norma;
    }
}
    ...
    for(int d=0; d<dims.size();d++){
        ...
        for (int funcid = 1; funcid <= 30; funcid++) {
            ...
            for(int s=0; s<n_ejecuciones; s++){       
                ...
                cec17_init("universe", funcid, dim);
                ...
                while(evals < max_evaluaciones ) {
                    ...
                    for(int i=0; i<universo.size(); i++){
                        ...
                        if(best_galaxy!=i && distancia<0.01 && universo.size()>10){
